...
(function() {
    ...
    document.getElementById("nextquestionbttn").addEventListener('click', function(event) { // 2nd arrugment invokes checkAnswer function on line 142
        checkAnswer();
    });

    document.getElementById("true").addEventListener('click', function(event) { //when true is clicked the checkAnswer(answer) is invoked on line 149
        checkAnswer(true);
    });
    document.getElementById("false").addEventListener('click', function(event) { // checks for false answer when clicked
        checkAnswer(false);
    });
    document.getElementById("nextquestionbttn").addEventListener('click', function(event) { // loads next question
        nextQuestion();
    });
    document.getElementById("resetquiz").addEventListener('click', function(event) { // resets game
        resetGame();
		finalScore();
    });
...
function nextQuestion() {
    document.getElementById('response').classList.add('hide'); // hides the response until true or false is click
    if (currentquestion < questions.length - 1) {
        currentquestion = currentquestion + 1; // current question is set to 0 on line 1 then loads in the next question
        document.getElementById('nextquestionbttn').classList.add('hide');
        document.getElementById('resetquiz').classList.add('hide');
        ...
        document.getElementById('resetquiz').classList.remove('hide');
    }
    document.getElementById('response').classList.add('hide');
    document.getElementById('riquestiontext').innerHTML = questions[currentquestion].question;
    //adding the iamge for each movie that correspondence to the current question
...
function checkAnswer(answer) {
    ...
    var questionAnswer = questions[currentquestion].answer; // creating new variable assign to question and using dot notation for grab answer 
